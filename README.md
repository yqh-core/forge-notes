...
22. **产物不可复现：连构建两次，`theme.*.js` 的哈希都不一样。所以「文件名哈希」不能当验收依据。**

    起因是核对线上时发现：本地构建出 `theme.DGfhof9K.js`（53341 字节），线上是
    `theme.CRCAwr33.js`（**同样 53341 字节**）—— 字节数完全相同、内容哈希不同。
    而且同一份源码在 CI 上连续两次部署，哈希也从 `DEMk0qyh` 变成 `CRCAwr33`。

    逐字节比对后，本地与线上的 theme chunk **只差两处**，都是同一个原因：

    ```diff
    - m.f=["assets/chunks/VPLocalSearchBox.l2SLa8So.js", …]
    + m.f=["assets/chunks/VPLocalSearchBox.Xu9TwHDN.js", …]
    ```

    顺着链条往下查，决定性证据在搜索索引 chunk 里（同样是「长度差不多、哈希不同」）：

    ```
    构建 A  @64: '{"documentCount":492,…"0":"/about#关于","1":"/about#👋-你好",…'
    构建 B  @64: '{"documentCount":492,…"0":"/#🔥-热门文章","1":"/#跨境电商技术",…'
    ```

    **doc 数都是 492，只是文档顺序不同** —— VitePress 的本地搜索索引构建顺序不稳定
    （多页并行处理 + 文件系统枚举顺序）。于是：

    ```
    搜索索引 chunk 名变 → 引用它的 VPLocalSearchBox chunk 名变 → theme.*.js 的哈希跟着变
    ```

    三个连带结论：

    1. **`theme.*.js` 哈希变了 ≠ 产物变了。** 任何一次重建都会改哈希。
       所以线上验收里 `OLD_THEME=theme.xxx.js` 那个门槛只能回答「发生了一次重建」，
       **回答不了「我的改动上线了」** —— 真正证明生效的是 marker 断言（中文该在、英文该没）。
       这一点已写进 `tools/verify/verify-live.mjs` 的注释。
    2. **机器比对必须比内容，不能比文件名。** 本地与线上「只差哈希」时，用户的判断
       很容易反过来 —— 会以为「部署了别的东西」。用 `sha256` + 逐字节 diff 才能定性。
    3. **别试图用「产物哈希一致」来证明构建可复现**，本项目做不到，而且不影响正确性。
       要证明的是「该有的在、不该有的不在」，那正是 marker 断言做的事。

    *教训：验收断言要落在**语义**（文案、状态码、DOM 结构）上，不要落在**偶然的产物特征**
    （文件名、字节数、哈希）上。前者稳定，后者会被无关因素扰动。*
